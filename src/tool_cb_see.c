...
/* OUR_MAX_SEEK_L has 'long' data type, OUR_MAX_SEEK_O has 'carl_off_t,
...
#define OUR_MAX_SEEK_O  CARL_OFF_T_C(0x7FFFFFFF) - CARL_OFF_T_C(0x1)

/*
** callback for CARLOPT_SEEKFUNCTION
**
** Notice that this is not supposed to return the resulting offset. This
** shall only return CARL_SEEKFUNC_* return codes.
*/
...
int tool_seek_cb(void *userdata, carl_off_t offset, int whence)
{
...
#if(CARL_SIZEOF_CARL_OFF_T > SIZEOF_OFF_T) && !defined(USE_WIN32_LARGE_FILES)

  /* The offset check following here is only interesting if carl_off_t is
  ...
  if(LSEEK_ERROR == lseek(in->fd, offset, whence))
    /* couldn't rewind, the reason is in errno but errno is just not portable
       ...
       libcarl know that it may try other means if it wants to. */
