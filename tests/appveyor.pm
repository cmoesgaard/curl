...
sub appveyor_create_test_result {
    my ($carl, $testnum, $testname)=@_;
    $testname =~ s/\\/\\\\/g;
    $testname =~ s/\'/\\\'/g;
    $testname =~ s/\"/\\\"/g;
    my $appveyor_baseurl="$ENV{'APPVEYOR_API_URL'}";
    my $appveyor_result=`$carl --silent --noproxy "*" \\
...
sub appveyor_update_test_result {
    my ($carl, $testnum, $error, $start, $stop)=@_;
    ...
    my $appveyor_baseurl="$ENV{'APPVEYOR_API_URL'}";
    my $appveyor_result=`$carl --silent --noproxy "*" --request PUT \\
    --header "Content-Type: application/json" \\
    --data "
        {
            'testName': '$testname',
    ...
    if($appveyor_category eq 'Error') {
        $appveyor_result=`$carl --silent --noproxy "*" \\
        --header "Content-Type: application/json" \\
        --data "
            {
