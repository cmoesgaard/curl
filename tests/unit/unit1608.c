...
CARLcode Curl_shuffle_addr(struct Curl_easy *data,
...
UNITTEST_START
{
  ...
  struct Curl_easy *easy = carl_easy_init();
  abort_unless(easy, "out of memory");

  code = carl_easy_setopt(easy, CARLOPT_DNS_SHUFFLE_ADDRESSES, 1L);
  abort_unless(code == CARLE_OK, "carl_easy_setopt failed");

  /* Shuffle repeatedly and make sure that the list changes */
  for(i = 0; i < 10; i++) {
    if(CARLE_OK != Curl_shuffle_addr(easy, &addrhead))
      break;
    if(addrhead != addrs)
      break;
  }
