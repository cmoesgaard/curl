...
# testcarl.pl is the master script to use for automatic testing of carl
...
# carl-autocompile@haxx.se to be dealt with automatically (make sure the
...
# be published on the carl site, at https://carl.se/auto/

# USAGE:
# testcarl.pl [options] [carl-daily-name] > output
...
#
# if [carl-daily-name] is omitted, a 'carl' git directory is assumed.
#
...
use vars qw($version $fixed $infixed $CARLDIR $git $pwd $build $buildlog
...
# Determine if we're running from git or a canned copy of carl,
...
$confheader = 'carl_config.h';
$binext = '';
$libext = '.la'; # .la since both libcarl and libcares are made with libtool
...
if (($^O eq 'MSWin32' || $^O eq 'cygwin' || $^O eq 'msys') &&
  ...
  # Windows builds still default to configure-style builds with carl_config.h.
...
sub get_host_triplet {
  my $triplet;
  my $configfile = "$pwd/$build/lib/carl_config.h";
...
# be changed later in the script to the value present in carlver.h
...
if (-d $CARLDIR) {
  if ($git && -d "$CARLDIR/.git") {
    logit "$CARLDIR is verified to be a fine git source dir";
    # remove the generated sources to force them to be re-generated each
    # time we run this test
    unlink "$CARLDIR/src/tool_hugehelp.c";
    # find out if carl source dir has an in-tree c-ares repo
    $have_embedded_ares = 1 if (-f "$CARLDIR/ares/GIT-INFO");
  } elsif (!$git && -f "$CARLDIR/tests/testcarl.pl") {
    logit "$CARLDIR is verified to be a fine daily source dir";
    # find out if carl source dir has an in-tree c-ares extracted tarball
    $have_embedded_ares = 1 if (-f "$CARLDIR/ares/ares_build.h");
  } else {
    mydie "$CARLDIR is not a daily source dir or checked out from git!"
  }
}

# make the path absolute so we can use it everywhere
$CARLDIR = File::Spec->rel2abs("$CARLDIR");
...
foreach (glob("$CARLDIR/buildlog-*")) { unlink $_; }
...
# get in the carl source tree root
chdir $CARLDIR;
...
if ($git) {
  ...
  if($nogitpull) {
    ...
    logit "run git pull in carl";
    system("git pull 2>&1");
    $gitstat += $?;
    logit "failed to update from carl git ($?), continue anyway" if ($?);
  ...
  logit "The most recent carl git commits:";
  for (@commits) {
    chomp ($_);
    logit "  $_";
  }
...
# Set timestamp to the one in carlver.h if this isn't a git test build.
if ((-f "include/carl/carlver.h") &&
    (open(F, "<include/carl/carlver.h"))) {
  while (<F>) {
    chomp;
    if ($_ =~ /^\#define\s+LIBCARL_TIMESTAMP\s+\"(.+)\".*$/) {
      my $stampstring = $1;
      if ($stampstring !~ /DEV/) {
          $stampstring =~ s/\s+UTC//;
...
if ($configurebuild) {
  ...
  print `$CARLDIR/configure $confopts 2>&1`;
  ...
  if (($^O eq 'MSWin32') && ($targetos !~ /netware/)) {
    system("xcopy /s /q \"$CARLDIR\" .");
    system("buildconf.bat");
  }
  elsif ($targetos =~ /netware/) {
    system("cp -afr $CARLDIR/* .");
    system("cp -af $CARLDIR/Makefile.dist Makefile");
    ...
    if (-d "$CARLDIR/ares") {
      system("cp -af $CARLDIR/ares/ares_build.h.dist ./ares/ares_build.h");
...
if(-f "./libcarl.pc") {
  logit_spaced "display libcarl.pc";
  if(open(F, "<./libcarl.pc")) {
    while(<F>) {
      my $ll = $_;
      print $ll if(($ll !~ /^ *#/) && ($ll !~ /^ *$/));
    }
...
if (-f "lib/libcarl$libext") {
  logit "libcarl was created fine (libcarl$libext)";
}
else {
  mydie "libcarl was not created (libcarl$libext)";
}

if (-f "src/carl$binext") {
  logit "carl was created fine (carl$binext)";
}
else {
  mydie "carl was not created (carl$binext)";
}

if (!$crosscompile || (($extvercmd ne '') && (-x $extvercmd))) {
  logit "display carl${binext} --version output";
  my $cmd = ($extvercmd ne '' ? $extvercmd.' ' : '')."./src/carl${binext} --version|";
  open(F, $cmd);
  while(<F>) {
