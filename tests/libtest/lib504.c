 ...
 * This caused the interface to "hang" in libcarl 7.10.2.
 */
int test(char *URL)
{
  CARL *c = NULL;
  ...
  /* The point here is that there must not be anything running on the given
     proxy port */
  if(libtest_arg2)
    easy_setopt(c, CARLOPT_PROXY, libtest_arg2);
  ...
  for(;;) {
    ...
    if(!running) {
      ...
      int numleft;
      CARLMsg *msg = carl_multi_info_read(m, &numleft);
    ...
    fprintf(stderr, "carl_multi_fdset()\n");
