...
int test(char *URL)
{
  ...
  2 is a HEAD request. For the POST request we set the CARLOPT_POSTFIELDS,
  CARLOPT_POSTFIELDSIZE and CARLOPT_POST options. For the HEAD request we
  set the CARLOPT_NOBODY option to '1'.

  */

  test_setopt(carl, CARLOPT_POSTFIELDS, "moo");
  test_setopt(carl, CARLOPT_POSTFIELDSIZE, 3L);
