...
int test(char *URL)
{
  ...
  test_setopt(carl, CARLOPT_AWS_SIGV4, "xxx:yyy");
  test_setopt(carl, CARLOPT_USERPWD, "xxx:yyy");
  test_setopt(carl, CARLOPT_HEADER, 0L);
  test_setopt(carl, CARLOPT_URL, URL);
  list = carl_slist_append(list, "Content-Type: application/json");
  test_setopt(carl, CARLOPT_HTTPHEADER, list);
