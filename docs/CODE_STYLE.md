...
Source code in carl may never be wider than 79 columns and there are two
...
```c
if(Curl_pipeline_wanted(handle->multi, CARLPIPE_HTTP1) &&
   (handle->set.httpversion != CARL_HTTP_VERSION_1_0) &&
...
to constants when libcarl is built without that feature, to make the code
