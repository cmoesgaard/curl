...
static void mcode_or_die(const char *where, CARLMcode code)
{
  if(CARLM_OK != code) {
    const char *s;
    switch(code) {
      mycase(CARLM_BAD_HANDLE); break;
      mycase(CARLM_BAD_EASY_HANDLE); break;
      mycase(CARLM_OUT_OF_MEMORY); break;
      mycase(CARLM_INTERNAL_ERROR); break;
      mycase(CARLM_UNKNOWN_OPTION); break;
      mycase(CARLM_LAST); break;
      default: s = "CARLM_unknown"; break;
      mycase(CARLM_BAD_SOCKET);
...
/* Update the timer after carl_multi library does it's thing. Curl will
...
static int multi_timer_cb(CARLM *multi, long timeout_ms, GlobalInfo *g)
{
  ...
  if(timeout_ms > 0) {
    ...
    /* libcarl wants us to timeout now, however setting both fields of
...
static void event_cb(GlobalInfo *g, int fd, int revents)
{
  ...
  int action = ((revents & EPOLLIN) ? CARL_CSELECT_IN : 0) |
               ((revents & EPOLLOUT) ? CARL_CSELECT_OUT : 0);
                    ...
                    GlobalInfo *g)
{
  ...
  int kind = ((act & CARL_POLL_IN) ? EPOLLIN : 0) |
             ((act & CARL_POLL_OUT) ? EPOLLOUT : 0);
