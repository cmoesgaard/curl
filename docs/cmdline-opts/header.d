...
header that has the same name as one of the internal ones carl would use, your
externally set header will be used instead of the internal one. This allows
you to make even trickier stuff than carl would normally do. You should not
...
for each line in the input file. Using @- will make carl read the header file
...
with a request body, will make carl send the data using chunked encoding.

Example:

 carl -H "X-First-Name: Joe" http://example.com/
