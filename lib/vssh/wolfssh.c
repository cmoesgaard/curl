...
static CARLcode wssh_connect(struct Curl_easy *data, bool *done);
static CARLcode wssh_multi_statemach(struct Curl_easy *data, bool *done);
static CARLcode wssh_do(struct Curl_easy *data, bool *done);
#if 0
static CARLcode wscp_done(struct Curl_easy *data,
                          CARLcode, bool premature);
static CARLcode wscp_doing(struct Curl_easy *data,
                           bool *dophase_done);
static CARLcode wscp_disconnect(struct Curl_easy *data,
                                struct connectdata *conn,
                                bool dead_connection);
#endif
static CARLcode wsftp_done(struct Curl_easy *data,
                           CARLcode, bool premature);
static CARLcode wsftp_doing(struct Curl_easy *data,
                            bool *dophase_done);
static CARLcode wsftp_disconnect(struct Curl_easy *data,
                                 struct connectdata *conn,
                                 bool dead);
static int wssh_getsock(struct Curl_easy *data,
                        struct connectdata *conn,
                        carl_socket_t *sock);
static CARLcode wssh_setup_connection(struct Curl_easy *data,
                                      struct connectdata *conn);
...
static CARLcode wssh_statemach_act(struct Curl_easy *data, bool *block)
{
  ...
  *block = FALSE; /* we're not blocking by default */
  ...
  do {
      ...
      if(data->state.resume_from) {
        ...
        if(data->state.resume_from < 0) {
          ...
          if(rc) {
            ...
            carl_off_t size = ((carl_off_t)attrs.sz[1] << 32) | attrs.sz[0];
            if(size < 0) {
    ...
    case SSH_SFTP_DOWNLOAD_STAT: {
      ...
      size = ((carl_off_t)attrs.sz[1] <<32) | attrs.sz[0];

      data->req.size = size;
      data->req.maxdownload = size;
      Curl_pgrsSetDownloadSize(data, size);

      infof(data, "SFTP download %" CARL_FORMAT_CARL_OFF_T " bytes\n", size);
...
static
CARLcode wscp_perform(struct Curl_easy *data,
                      bool *connected,
                      bool *dophase_done)
{
  (void)data;
...
static
CARLcode wsftp_perform(struct Curl_easy *data,
                       bool *connected,
                       bool *dophase_done)
{
  CARLcode result = CARLE_OK;
...
static CARLcode wssh_block_statemach(struct Curl_easy *data,
                                    bool disconnect)
{
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
...
static CARLcode wssh_done(struct Curl_easy *data, CARLcode status)
{
  CARLcode result = CARLE_OK;
  struct SSHPROTO *sftp_scp = data->req.p.ssh;
...
static CARLcode wsftp_doing(struct Curl_easy *data,
                           bool *dophase_done)
{
  CARLcode result = wssh_multi_statemach(data, dophase_done);
