...
{
  if(ntlm->ntlm_auth_hlpr_socket != CARL_SOCKET_BAD) {
    sclose(ntlm->ntlm_auth_hlpr_socket);
    ntlm->ntlm_auth_hlpr_socket = CARL_SOCKET_BAD;
  }
...
static CARLcode ntlm_wb_init(struct Curl_easy *data, struct ntlmdata *ntlm,
                             const char *userp)
{
  carl_socket_t sockfds[2];
  pid_t child_pid;
  const char *username;
  ...
  /* For testing purposes, when DEBUGBUILD is defined and environment
     variable CARL_NTLM_WB_FILE is set a fake_ntlm is used to perform
     NTLM challenge/response which only accepts commands and output
     strings pre-written in test case definitions */
#ifdef DEBUGBUILD
  ntlm_auth_alloc = carl_getenv("CARL_NTLM_WB_FILE");
  if(ntlm_auth_alloc)
...
static CARLcode ntlm_wb_response(struct Curl_easy *data, struct ntlmdata *ntlm,
                                 const char *input, carlntlm state)
{
...
CARLcode Curl_input_ntlm_wb(struct Curl_easy *data,
                            struct connectdata *conn,
                            bool proxy,
                            const char *header)
{
  struct ntlmdata *ntlm = proxy ? &conn->proxyntlm : &conn->ntlm;
  carlntlm *state = proxy ? &conn->proxy_ntlm_state : &conn->http_ntlm_state;
 ...
 */
CARLcode Curl_output_ntlm_wb(struct Curl_easy *data, struct connectdata *conn,
                             bool proxy)
{
...
#endif /* !CARL_DISABLE_HTTP && USE_NTLM && NTLM_WB_ENABLED */
