                    ...
                    struct Curl_addrinfo **result)
{
  ...
  *result = NULL; /* assume failure */
  ...
  for(ai = aihead; ai != NULL; ai = ai->ai_next) {
    ...
    ca->ai_addrlen   = (carl_socklen_t)ss_size;
    ca->ai_addr      = NULL;
 ...
 *
 *   Curl_addrinfo defined in "lib/carl_addrinfo.h"
 *
 ...
 *       int                   ai_socktype;
 *       int                   ai_protocol;
 *       carl_socklen_t        ai_addrlen;   * Follow rfc3493 struct addrinfo *
 *       char                 *ai_canonname;
...
Curl_he2ai(const struct hostent *he, int port)
{
  ...
  for(i = 0; (curr = he->h_addr_list[i]) != NULL; i++) {
    ...
    case AF_INET:
      ...
      addr->sin_family = (CARL_SA_FAMILY_T)(he->h_addrtype);
    ...
    case AF_INET6:
      ...
      addr6->sin6_family = (CARL_SA_FAMILY_T)(he->h_addrtype);
...
#if defined(CARLDEBUG) && defined(HAVE_GETADDRINFO) &&  \
  defined(HAVE_FREEADDRINFO)
/*
 * carl_dbg_freeaddrinfo()
 *
...
void
carl_dbg_freeaddrinfo(struct addrinfo *freethis,
                      int line, const char *source)
{
  carl_dbg_log("ADDR %s:%d freeaddrinfo(%p)\n",
...
#endif /* defined(CARLDEBUG) && defined(HAVE_FREEADDRINFO) */


#if defined(CARLDEBUG) && defined(HAVE_GETADDRINFO)
/*
 * carl_dbg_getaddrinfo()
 *
 * This is strictly for memory tracing and are using the same style as the
 * family otherwise present in memdebug.c. I put these ones here since they
 * require a bunch of structs I didn't want to include in memdebug.c
...
int
carl_dbg_getaddrinfo(const char *hostname,
                    const char *service,
                    const struct addrinfo *hints,
                    ...
                    int line, const char *source)
{
  ...
  if(0 == res)
    /* success */
    carl_dbg_log("ADDR %s:%d getaddrinfo() = %p\n",
...
#endif /* defined(CARLDEBUG) && defined(HAVE_GETADDRINFO) */
