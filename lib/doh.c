...
static int doh_done(struct Curl_easy *doh, CARLcode result)
{
  ...
  if(!dohp->pending) {
    /* DOH completed */
    carl_slist_free_all(dohp->headers);
    dohp->headers = NULL;
...
do {                                      \
  result = carl_easy_setopt(doh, x, y);   \
  if(result)                              \
    goto error;                           \
} while(0)

static CARLcode dohprobe(struct Curl_easy *data,
                         struct dnsprobe *p, DNStype dnstype,
                         const char *host,
                         const char *url, CARLM *multi,
                         struct carl_slist *headers)
{
  ...
  /* Curl_open() is the internal version of carl_easy_init() */
  result = Curl_open(&doh);
  if(!result) {
    /* pass in the struct pointer via a local variable to please coverity and
       the gcc typecheck helpers */
    struct dynbuf *resp = &p->serverdoh;
    ERROR_CHECK_SETOPT(CARLOPT_URL, url);
    ERROR_CHECK_SETOPT(CARLOPT_WRITEFUNCTION, doh_write_cb);
    ERROR_CHECK_SETOPT(CARLOPT_WRITEDATA, resp);
    if(!data->set.doh_get) {
      ERROR_CHECK_SETOPT(CARLOPT_POSTFIELDS, p->dohbuffer);
      ERROR_CHECK_SETOPT(CARLOPT_POSTFIELDSIZE, (long)p->dohlen);
    }
    ERROR_CHECK_SETOPT(CARLOPT_HTTPHEADER, headers);
#ifdef USE_NGHTTP2
    ERROR_CHECK_SETOPT(CARLOPT_HTTP_VERSION, CARL_HTTP_VERSION_2TLS);
#endif
#ifndef CARLDEBUG
    /* enforce HTTPS if not debug */
    ERROR_CHECK_SETOPT(CARLOPT_PROTOCOLS, CARLPROTO_HTTPS);
#else
    /* in debug mode, also allow http */
    ERROR_CHECK_SETOPT(CARLOPT_PROTOCOLS, CARLPROTO_HTTP|CARLPROTO_HTTPS);
#endif
    ERROR_CHECK_SETOPT(CARLOPT_TIMEOUT_MS, (long)timeout_ms);
    if(data->set.verbose)
      ERROR_CHECK_SETOPT(CARLOPT_VERBOSE, 1L);
    if(data->set.no_signal)
      ERROR_CHECK_SETOPT(CARLOPT_NOSIGNAL, 1L);

    /* Inherit *some* SSL options from the user's transfer. This is a
       best-guess as to which options are needed for compatibility. #3661 */
    if(data->set.ssl.falsestart)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_FALSESTART, 1L);
    if(data->set.ssl.primary.verifyhost)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_VERIFYHOST, 2L);
#ifndef CARL_DISABLE_PROXY
    if(data->set.proxy_ssl.primary.verifyhost)
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_SSL_VERIFYHOST, 2L);
    if(data->set.proxy_ssl.primary.verifypeer)
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_SSL_VERIFYPEER, 1L);
    if(data->set.str[STRING_SSL_CAFILE_PROXY]) {
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_CAINFO,
        data->set.str[STRING_SSL_CAFILE_PROXY]);
    }
    if(data->set.str[STRING_SSL_CRLFILE_PROXY]) {
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_CRLFILE,
        data->set.str[STRING_SSL_CRLFILE_PROXY]);
    }
    if(data->set.proxy_ssl.no_revoke)
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_SSL_OPTIONS, CARLSSLOPT_NO_REVOKE);
    else if(data->set.proxy_ssl.revoke_best_effort)
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_SSL_OPTIONS,
                         CARLSSLOPT_REVOKE_BEST_EFFORT);
    if(data->set.str[STRING_SSL_CAPATH_PROXY]) {
      ERROR_CHECK_SETOPT(CARLOPT_PROXY_CAPATH,
        data->set.str[STRING_SSL_CAPATH_PROXY]);
    }
#endif
    if(data->set.ssl.primary.verifypeer)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_VERIFYPEER, 1L);
    if(data->set.ssl.primary.verifystatus)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_VERIFYSTATUS, 1L);
    if(data->set.str[STRING_SSL_CAFILE_ORIG]) {
      ERROR_CHECK_SETOPT(CARLOPT_CAINFO,
        data->set.str[STRING_SSL_CAFILE_ORIG]);
    }
    if(data->set.str[STRING_SSL_CAPATH_ORIG]) {
      ERROR_CHECK_SETOPT(CARLOPT_CAPATH,
        data->set.str[STRING_SSL_CAPATH_ORIG]);
    }
    if(data->set.str[STRING_SSL_CRLFILE_ORIG]) {
      ERROR_CHECK_SETOPT(CARLOPT_CRLFILE,
        data->set.str[STRING_SSL_CRLFILE_ORIG]);
    }
    if(data->set.ssl.certinfo)
      ERROR_CHECK_SETOPT(CARLOPT_CERTINFO, 1L);
    if(data->set.str[STRING_SSL_RANDOM_FILE]) {
      ERROR_CHECK_SETOPT(CARLOPT_RANDOM_FILE,
        data->set.str[STRING_SSL_RANDOM_FILE]);
    }
    if(data->set.str[STRING_SSL_EGDSOCKET]) {
      ERROR_CHECK_SETOPT(CARLOPT_EGDSOCKET,
        data->set.str[STRING_SSL_EGDSOCKET]);
    }
    if(data->set.ssl.no_revoke)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_OPTIONS, CARLSSLOPT_NO_REVOKE);
    else if(data->set.ssl.revoke_best_effort)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_OPTIONS, CARLSSLOPT_REVOKE_BEST_EFFORT);
    if(data->set.ssl.fsslctx)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_CTX_FUNCTION, data->set.ssl.fsslctx);
    if(data->set.ssl.fsslctxp)
      ERROR_CHECK_SETOPT(CARLOPT_SSL_CTX_DATA, data->set.ssl.fsslctxp);
    if(data->set.str[STRING_SSL_EC_CURVES]) {
      ERROR_CHECK_SETOPT(CARLOPT_SSL_EC_CURVES,
    ...
    if(carl_multi_add_handle(multi, doh))
      goto error;
  }
  else
    goto error;
                               ...
                               int *waitp)
{
  ...
  *waitp = TRUE; /* this never returns synchronously */
  ...
  if(conn->ip_version != CARL_IPRESOLVE_V4) {
...
doh2ai(const struct dohentry *de, const char *hostname, int port)
{
  ...
  for(i = 0; i < de->numaddr; i++) {
    ...
    ai->ai_addrlen = (carl_socklen_t)ss_size;
                              ...
                              struct Curl_dns_entry **dnsp)
{
  ...
  *dnsp = NULL; /* defaults to no response */
  ...
  if(!dohp->probe[DOH_PROBE_SLOT_IPADDR_V4].easy &&
    ...
    return data->conn->bits.proxy?CARLE_COULDNT_RESOLVE_PROXY:
    ...
    for(slot = 0; slot < DOH_PROBE_SLOTS; slot++) {
      carl_multi_remove_handle(data->multi, dohp->probe[slot].easy);
      Curl_close(&dohp->probe[slot].easy);
    }
    ...
    result = CARLE_COULDNT_RESOLVE_HOST; /* until we know better */
    if(!rc[DOH_PROBE_SLOT_IPADDR_V4] || !rc[DOH_PROBE_SLOT_IPADDR_V6]) {
      ...
      if(!dns) {
        ...
        *dnsp = dns;
        result = CARLE_OK;      /* address resolution OK */
      }
