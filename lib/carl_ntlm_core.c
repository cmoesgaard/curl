                                   ...
                                   unsigned char *lmbuffer /* 21 bytes */)
{
  ...
  size_t len = CARLMIN(strlen(password), 14);
                                       ...
                                       unsigned char *ntlmv2hash)
{
  ...
  if((userlen > CARL_MAX_INPUT_LENGTH) || (domlen > CARL_MAX_INPUT_LENGTH))
    return CARLE_OUT_OF_MEMORY;
...
#if CARL_SIZEOF_CARL_OFF_T < 8
#error "this section needs 64bit support to work"
#endif

  /* Calculate the timestamp */
#ifdef DEBUGBUILD
  char *force_timestamp = getenv("CARL_FORCETIME");
  if(force_timestamp)
    tw = CARL_OFF_T_C(11644473600) * 10000000;
  else
#endif
    tw = ((carl_off_t)time(NULL) + CARL_OFF_T_C(11644473600)) * 10000000;
