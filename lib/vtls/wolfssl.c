...
/* To determine what functions are available we rely on one or both of:
   ...
   - the symbols detected by carl's configure
...
wolfssl_log_tls12_secret(SSL *ssl)
{
   ...
   * carl rejects the server certificate and aborts the handshake.
   */
  for(i = 0; i < msLen; i++) {
    x |= ms[i];
  }
...
#define use_sni(x)  Curl_nop_stmt
  ...
  if(data->set.ssl.fsslctx) {
    CARLcode result = (*data->set.ssl.fsslctx)(data, backend->ctx,
    ...
    failf(data, "SSL: Certificates can't be loaded because wolfSSL was built"
          ...
          " (insecure) or if you are building an application with libcarl you"
          " can load certificates via CARLOPT_SSL_CTX_FUNCTION.");
    return CARLE_SSL_CONNECT_ERROR;
  }
#endif
...
#ifdef USE_NGHTTP2
      else if(data->set.httpversion >= CARL_HTTP_VERSION_2 &&
...
static CARLcode wolfssl_connect_nonblocking(struct Curl_easy *data,
                                            struct connectdata *conn,
...
static CARLcode wolfssl_connect(struct Curl_easy *data,
...
static CARLcode wolfssl_random(struct Curl_easy *data,
...
static CARLcode wolfssl_sha256sum(const unsigned char *tmp, /* input */
                                  size_t tmplen,
...
const struct Curl_ssl Curl_ssl_wolfssl = {
  { CARLSSLBACKEND_WOLFSSL, "WolfSSL" }, /* info */
