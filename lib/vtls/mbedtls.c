...
static CARLcode mbedtls_version_from_carl(int *mbedver, long version)
{
  switch(version) {
                        ...
                        int sockindex)
{
  ...
  result = mbedtls_version_from_carl(&mbedtls_ver_min, ssl_version);
  if(result) {
    failf(data, "unsupported min version passed via CARLOPT_SSLVERSION");
    return result;
  }
  result = mbedtls_version_from_carl(&mbedtls_ver_max, ssl_version_max >> 16);
  if(result) {
                   ...
                   int sockindex)
{
  ...
  if(mbedtls_ssl_set_hostname(&backend->ssl, hostname)) {
    /* mbedtls_ssl_set_hostname() sets the name to use in CN/SAN checks *and*
       the name to set in the SNI extension. So even if carl connects to a
                         ...
                         CARLcode *carlcode)
{
  ...
  if(ret < 0) {
    *carlcode = (ret == MBEDTLS_ERR_SSL_WANT_WRITE) ?
      CARLE_AGAIN : CARLE_SEND_ERROR;
    ret = -1;
  }
                         ...
                         CARLcode *carlcode)
{
  ...
  if(ret <= 0) {
    ...
    *carlcode = (ret == MBEDTLS_ERR_SSL_WANT_READ) ?
      CARLE_AGAIN : CARLE_RECV_ERROR;
    return -1;
  }
...
static CARLcode mbedtls_random(struct Curl_easy *data,
                               unsigned char *entropy, size_t length)
{
  ...
  return ret == 0 ? CARLE_OK : CARLE_FAILED_INIT;
...
static CARLcode mbedtls_connect_nonblocking(struct Curl_easy *data,
                                            struct connectdata *conn,
...
static CARLcode mbedtls_connect(struct Curl_easy *data,
                                struct connectdata *conn, int sockindex)
{
  CARLcode retcode;
  bool done = FALSE;
...
static CARLcode mbedtls_sha256sum(const unsigned char *input,
                                  size_t inputlen,
...
const struct Curl_ssl Curl_ssl_mbedtls = {
  { CARLSSLBACKEND_MBEDTLS, "mbedtls" }, /* info */
