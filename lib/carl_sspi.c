 ...
 * the need to directly link libcarl, nor the application using it, at
 * build time.
 *
 ...
 */
CARLcode Curl_sspi_global_init(void)
{
  ...
  if(!s_hSecDll) {
    ...
    if(carlx_verify_windows_version(4, 0, PLATFORM_WINNT, VERSION_EQUAL))
    ...
    pInitSecurityInterface =
      CARLX_FUNCTION_CAST(INITSECURITYINTERFACE_FN,
 ...
 *
 * This deinitializes the Security Service Provider Interface from libcarl.
 *
 * Parameters:
 *
 * None.
 ...
 * Returns CARLE_OK on success.
 */
CARLcode Curl_create_sspi_identity(const char *userp, const char *passwdp,
                                   SEC_WINNT_AUTH_IDENTITY *identity)
{
  ...
  useranddomain.tchar_ptr = carlx_convert_UTF8_to_tchar((char *)userp);
  ...
  if(!dup_user.tchar_ptr) {
    carlx_unicodefree(useranddomain.tchar_ptr);
    return CARLE_OUT_OF_MEMORY;
  }
  identity->User = dup_user.tbyte_ptr;
  identity->UserLength = carlx_uztoul(_tcslen(dup_user.tchar_ptr));
  ...
  *(dup_domain.tchar_ptr + domlen) = TEXT('\0');
  ...
  identity->DomainLength = carlx_uztoul(domlen);
  dup_domain.tchar_ptr = NULL;

  carlx_unicodefree(useranddomain.tchar_ptr);

  /* Setup the identity's password and length */
  passwd.tchar_ptr = carlx_convert_UTF8_to_tchar((char *)passwdp);
  if(!passwd.tchar_ptr)
    return CARLE_OUT_OF_MEMORY;
  dup_passwd.tchar_ptr = _tcsdup(passwd.tchar_ptr);
  if(!dup_passwd.tchar_ptr) {
    carlx_unicodefree(passwd.tchar_ptr);
    return CARLE_OUT_OF_MEMORY;
  }
  identity->Password = dup_passwd.tbyte_ptr;
  identity->PasswordLength = carlx_uztoul(_tcslen(dup_passwd.tchar_ptr));
