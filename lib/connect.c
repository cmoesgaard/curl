...
static bool verifyconnect(carl_socket_t sockfd, int *error);
             ...
             carl_socket_t sockfd)
{
  ...
  if(setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE,
    ...
    DWORD dummy;
    vals.onoff = 1;
    optval = carlx_sltosi(data->set.tcp_keepidle);
    KEEPALIVE_FACTOR(optval);
    vals.keepalivetime = optval;
    optval = carlx_sltosi(data->set.tcp_keepintvl);
    KEEPALIVE_FACTOR(optval);
...
static CARLcode bindlocal(struct Curl_easy *data,
                          carl_socket_t sockfd, int af, unsigned int scope)
{
  struct connectdata *conn = data->conn;
  struct Curl_sockaddr_storage sa;
  struct sockaddr *sock = (struct sockaddr *)&sa;  /* bind to this address */
  carl_socklen_t sizeof_sa = 0; /* size of the data sock points to */
  ...
  if(dev && (strlen(dev)<255) ) {
    ...
    if(!is_host) {
      ...
      if(setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE,
                    dev, (carl_socklen_t)strlen(dev) + 1) == 0) {
    ...
    if(!is_interface) {
      ...
      if(af == AF_INET)
        conn->ip_version = CARL_IPRESOLVE_V4;
#ifdef ENABLE_IPV6
      else if(af == AF_INET6)
        conn->ip_version = CARL_IPRESOLVE_V6;
#endif
  ...
  for(;;) {
    if(bind(sockfd, sock, sizeof_sa) >= 0) {
      ...
      carl_socklen_t size = sizeof(add);
...
#ifdef SO_ERROR
  int err = 0;
  carl_socklen_t errSize = sizeof(err);
...
static CARLcode trynextip(struct Curl_easy *data,
                          struct connectdata *conn,
                          int sockindex,
                          int tempindex)
{
  CARLcode result = CARLE_COULDNT_CONNECT;

  /* First clean up after the failed socket.
     Don't close it yet to ensure that the next IP's socket gets a different
     file descriptor, which can prevent bugs when the carl_multi_socket_action
     interface is used with certain select() replacements such as kqueue. */
  carl_socket_t fd_to_close = conn->tempsock[tempindex];
  conn->tempsock[tempindex] = CARL_SOCKET_BAD;
  ...
  if(sockindex == FIRSTSOCKET) {
    ...
    while(ai) {
      if(ai) {
        ...
        if(result == CARLE_COULDNT_CONNECT) {
  ...
  if(fd_to_close != CARL_SOCKET_BAD)
...
#if defined(HAVE_SYS_UN_H) && defined(AF_UNIX)
    case AF_UNIX:
      if(salen > (carl_socklen_t)sizeof(sa_family_t)) {
...
void Curl_conninfo_remote(struct Curl_easy *data,
                          struct connectdata *conn, carl_socket_t sockfd)
{
...
*/
static CARLcode connect_SOCKS(struct Curl_easy *data, int sockindex,
                              bool *done)
{
  CARLcode result = CARLE_OK;
#ifndef CARL_DISABLE_PROXY
  CARLproxycode pxresult = CARLPX_OK;
  struct connectdata *conn = data->conn;
                           ...
                           bool *connected)
{
  ...
  *connected = FALSE; /* a very negative world view is best */
  ...
  for(i = 0; i<2; i++) {
    ...
    if(conn->transport == TRNSPRT_QUIC) {
      ...
      if(!result && *connected) {
        ...
        conn->tempsock[i] = CARL_SOCKET_BAD;
        post_SOCKS(data, conn, sockindex, connected);
    ...
    if(rc == 0) { /* no connection yet */
      if(Curl_timediff(now, conn->connecttime) >=
        ...
        infof(data, "After %" CARL_FORMAT_TIMEDIFF_T
    ...
    else if(rc == CARL_CSELECT_OUT || conn->bits.tcp_fastopen) {
      if(verifyconnect(conn->tempsock[i], &error)) {
        ...
        if(conn->tempsock[other] != CARL_SOCKET_BAD) {
          Curl_closesocket(data, conn, conn->tempsock[other]);
          conn->tempsock[other] = CARL_SOCKET_BAD;
        }
    ...
    if(error) {
      ...
      if(conn->tempaddr[i]) {
        ...
        if((status != CARLE_COULDNT_CONNECT) ||
           conn->tempsock[other] == CARL_SOCKET_BAD)
  ...
  if(result &&
     (conn->tempsock[0] == CARL_SOCKET_BAD) &&
     (conn->tempsock[1] == CARL_SOCKET_BAD)) {
    ...
    result = CARLE_OK; /* still trying */

  return result;
}

static void tcpnodelay(struct Curl_easy *data, carl_socket_t sockfd)
{
#if defined(TCP_NODELAY)
  carl_socklen_t onoff = (carl_socklen_t) 1;
...
{
  int val = CARL_MAX_WRITE_SIZE + 32;
  int curval = 0;
  int carlen = sizeof(curval);
  ...
  if(getsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (char *)&curval, &carlen) == 0)
    if(curval > val)
      return;
 ...
 * singleipconnect()
 *
 * Note that even on connect fail it returns CARLE_OK, but with 'sock' set to
 * CARL_SOCKET_BAD. Other errors will however return proper errors.
 *
 * singleipconnect() connects to the given IP only, and it may return without
 * having connected.
 */
static CARLcode singleipconnect(struct Curl_easy *data,
                                struct connectdata *conn,
...
#endif
  char buffer[STRERROR_LEN];
  carl_socket_t *sockp = &conn->tempsock[tempindex];
  *sockp = CARL_SOCKET_BAD;
  ...
  if(data->set.fsockopt) {
    ...
    if(error == CARL_SOCKOPT_ALREADY_CONNECTED)
      isconnected = TRUE;
    else if(error) {
  ...
  (void)carlx_nonblock(sockfd, TRUE);
                          ...
                          const struct Curl_dns_entry *remotehost)
{
  ...
  conn->tempsock[0] = conn->tempsock[1] = CARL_SOCKET_BAD;
                                  ...
                                  struct connectdata **connp)
{
   ...
   * - that has been used for carl_easy_perform()
   * - that is associated with a multi handle, and whose connection
   *   was detached with CARLOPT_CONNECT_ONLY
   */
...
#ifdef MSG_PEEK
  else if(conn->sock[FIRSTSOCKET] == CARL_SOCKET_BAD)
    return false;
  else {
    /* use the socket */
    char buf;
                     ...
                     carl_socket_t *sockfd)
{
   ...
   * carl_sockaddr structure with enough space available to directly hold
  ...
  if(data->set.fopensocket) {
    ...
    * CARL_SOCKET_BAD; otherwise it will return a not-connected socket. When
...
bool Curl_conn_data_pending(struct connectdata *conn, int sockindex)
{
  ...
  return (readable > 0 && (readable & CARL_CSELECT_IN));
}
