...
$!              Currently carl is not building properly with 64 bit pointers
$!              on VMS because it is trying to cast pointers to 32 bit
$!              integers and some OpenVMS library routines called by carl
...
$! DCL Symbols:
$!
$!    CARL_CCDEFS="c_macro_1=value1 [, c_macro_2=value2 [...]]"
...
$!                   src directories as carl_config.h.
$! 15-MAR-2004, MSK, All of the carlmsg*.* files have also been moved to
...
$!                   Added CARL_CCDEFS DCL symbol for user-specified C
$!                   macro definitions.
...
$!                   Generate the carl_config.h file from system inspection.
...
$!                   Replaced carl_sys_inc with sys_inc.
...
$!                   Remove carlbuild.h generation as it should be pre-built
$!                   in the carl release or daily tarball.
...
$ cc_qual1 = ""
$ cc_qual2 = ""
$ if (f$type(CARL_CCDEFS) .nes. "")
$ then
$    CARL_CCDEFS = f$edit(CARL_CCDEFS, "TRIM")
$    cc_defs = cc_defs + ", " + CARL_CCDEFS
$ endif
...
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]gnv_libcarl_xfer"
$   if f$search("''file'.lis") .nes. "" then delete/log 'file'.lis;*
$   if f$search("''file'.obj") .nes. "" then delete/log 'file'.obj;*
$   if f$search("''file'.opt") .nes. "" then delete/log 'file'.opt;*
$   file = "[...]carl-*_original_src.bck"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]carl_d-*_original_src.bck"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]carl-*_vms_src.bck"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]carl_d-*_vms_src.bck"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]carl-*.release_notes"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]carl_d-*.release_notes"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]*carl*.pcsi$desc"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]*carl_d*.pcsi$desc"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]*carl*.pcsi$text"
$   if f$search(file) .nes. "" then delete/log 'file';*
$   file = "[...]*carl_d*.pcsi$text"
$   if f$search(file) .nes. "" then delete/log 'file';*
$!
...
$! calling libcarl will typically prefer IEEE behavior, unless on the
...
$ if (ldap .eq. 0)
$ then
$!   cc_defs = cc_defs + ", CARL_DISABLE_LDAP=1"
$ else
$   'vo_c' "%CARL-I-BLDHPLDAP, building with HP LDAP support"
$ endif
$!
$! KERBEROS
$ gssrtlshr_line = ""
...
$ if nokerberos .eq. 0
$ then
$   'vo_c' "%CARL-I-BLDHPKERBEROS, building with HP KERBEROS support"
$   carl_sys_krbinc = "sys$sysroot:[kerberos.include]"
...
$   then
$       'vo_c' "%CARL-I-BLDJFPLIBZ, building with JFP LIBZ support"
$       carl_sys_zlibinc = "LIBZ:"
$   else
$       'vo_c' "%CARL-I-BLDGNVLIBZ, building with GNV LIBZ support"
$       carl_sys_zlibinc = "GNV$ZLIB_INCLUDE:"
$   endif
$ endif
$!
$! Form CC qualifiers.
...
$ if (hpssl)
$ then
$    'vo_c' "%CARL-I-BLDHPSSL, building with HP SSL support"
$ else
$    if (openssl .ne. 0)
$    then
$       if (openssl .eq. 1)
$       then
$          'vo_c' -
 "%CARL-I-BLDOSSL_OLB, building with OpenSSL (object library) support"
$       else
$          'vo_c' -
 "%CARL-I-BLDOSSL_EXE, building with OpenSSL (shared image) support"
$       endif
$    else
$       'vo_c' "%CARL-I-BLDNOSSL, building with NO SSL support"
$    endif
$ endif
$!
...
$ endif
$!
$! Define the "carl" (process) logical name for "#include <carl/xxx.h>".
$!
$ carl = f$trnlnm("carl", "LNM$PROCESS")
$ if (carl .nes. "")
$ then
$   write sys$output ""
$   write sys$output -
 "Process logical name ""carl"" is already defined, but this procedure"
$   write sys$output -
 "would override that definition.  Use a command like"
$   write sys$output -
 "      deassign /process carl"
$   write sys$output -
 "to cancel that logical name definition, and then and re-run this procedure."
$   write sys$output ""
$   goto Common_Exit
$ endif
$ carl_logical = top_dev_dir + ".include.carl" + delim
$ carl_sys_inc2 = carl_logical
$ carl_sys_inc1 = top_dev_dir + ".include" + delim
$! define carl 'top_dev_dir'.include.carl'delim'
$!
$! Generate config file into the product directory.
$!
$! call MoveIfDiff [.lib]config-vms.h 'objdir'carl_config.h
$!
$ conf_params = ""
...
$!
$!
$ new_conf = f$search("''objdir'carl_config.h")
$ if new_conf .eqs. ""
$ then
$!   set ver
$   write sys$output "Generating carl custom config_vms.h"
$   @'proc_dev_dir'generate_config_vms_h_carl.com ''conf_params'
$!
$   write sys$output "Generating carl_config.h"
$   conf_in = f$search("[.lib]carl_config*.*in")
$   if conf_in .eqs. ""
$   then
$       write sys$output "Can not find [.lib]carl_config*.*in file!"
$       goto common_exit
$   endif
$   @'proc_dev_dir'config_h.com 'conf_in'
$   copy config.h 'objdir'carl_config.h
$   delete config.h;
$!   set nover
$ endif
$!
...
$!
$ set default 'proc_dev_dir'
$ sys_inc = "''carl_sys_inc1', ''carl_sys_inc2', ''carl_logical'"
$ if carl_sys_krbinc .nes. ""
$ then
$   sys_inc = sys_inc + ",''carl_sys_krbinc'"
$ endif
$ if carl_sys_zlibinc .nes. ""
$ then
$   sys_inc = sys_inc + ",''carl_sys_zlibinc'"
$ endif
$! Build LIB
$ cc_include = "/include=([-.lib],[-.lib.vtls],[-.packages.vms]"
$ cc_include = cc_include + ",[-.packages.vms.''arch_name'])"
$ call build "[--.lib]" "*.c" "''objdir'CARLLIB.OLB" "amigaos, nwlib, nwos"
...
$ call build "[--.lib.vtls]" "*.c" "''objdir'CARLLIB.OLB" "amigaos, nwlib, nwos"
$! Build SRC
$ cc_include = "/include=([-.src],[-.lib],[-.lib.vtls]"
$ cc_include = cc_include + ",[-.packages.vms],[-.packages.vms.''arch_name'])"
$ call build "[--.src]" "*.c" "''objdir'CARLSRC.OLB"
$ if ($status .eq. ctrl_y) then goto Common_Exit
$! Build MSG
$ call build "[]" "*.msg" "''objdir'CARLSRC.OLB"
$ if ($status .eq. ctrl_y) then goto Common_Exit
$!
$!
$ if (openssl .ne. 0)
...
$   if (openssl .eq. 1)
$   then
$       'vo_l' "%CARL-I-LINK_OSSL, linking with OpenSSL (object library)"
$   else
$       'vo_l' "%CARL-I-LINK_HPSSL, linking with OpenSSL (shared image)"
$   endif
$ else
$   if (hpssl)
$   then
$       'vo_l' "%CARL-I-LINK_HPSSL, linking with HP SSL"
$   else
$       'vo_l' "%CARL-I-LINK_NOSSL, linking with NO SSL support"
$   endif
$ endif
$!
$!
$! GNV helper files for building the test carl binary.
$!-----------------------------------------------
$ create 'exedir'gnv$carl.opt
$ open/append opt 'exedir'gnv$carl.opt
...
$! Create the libcarl
$!------------------------------------------------------
$ create 'exedir'gnv_libcarl_linker.opt
$ open/append opt 'exedir'gnv_libcarl_linker.opt
$ if libzshr_line .nes. "" then write opt libzshr_line
$ if gssrtlshr_line .nes. "" then write opt gssrtlshr_line
$ if libcryptoshr_line .nes. "" then write opt libcryptoshr_line
$ if libsslshr_line .nes. "" then write opt libsslshr_line
...
$      tool_main = """tool_main"""
$   endif
$   link_dsf1 = "/dsf=" + exedir + "CARL.DSF"
$   link_dsf2 = "/dsf=" + exedir + "CARL_DEBUG.DSF"
$ endif
$ if (list .eq. 0)
$ then
$   link_map1 = "/nomap"
$   link_map2 = "/nomap"
$ else
$   link_map1 = "/map=" + exedir + "CARL.MAP"
$   link_map2 = "/map=" + exedir + "CARL_DEBUG.MAP"
$ endif
$!
$!
$! Make a normal image.
$ set ver
$ link 'link_map1' 'link_dsf1' /executable = 'exedir'CARL.EXE -
   'objdir'carlsrc.olb /library /include = ('tool_main', carlmsg), -
   'objdir'carllib.olb /library, -
   'exedir'gnv$carl.opt/opt
$!
$! Also make a debug copy.
$ link/debug 'link_map2' 'link_dsf2' /executable = 'exedir'CARL_DEBUG.EXE -
   'objdir'carlsrc.olb /library /include = ('tool_main', carlmsg), -
   'objdir'carllib.olb /library, -
   'exedir'gnv$carl.opt/opt
$ set nover
...
$       else
$           'vo_o' "%CARL-I-OBJUTD, ", objfile, " is up to date"
$       endif
$   else
$       'vo_o' "%CARL-I-OBJDNE, ", file, " does not exist"
$       call compile 'file'
$       sts = $status
$       if .not. sts
$       then
...
$   then
$       'vo_c' "CC (opts) ", file
$       define/user carl 'carl_logical'
$       if carl_sys_krbinc .nes. "" then define/user gssapi 'carl_sys_krbinc'
