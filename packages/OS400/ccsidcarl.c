...
#define ALLOC_GRANULE   8       /* Alloc. granule for carl_formadd_ccsid(). */
...
makeOS400IconvCode(char buf[ICONV_ID_SIZE], unsigned int ccsid)
{
  ...
  **/
  ...
  carl_msprintf(buf, "IBMCCSID%05u0000000", ccsid);
}
...
static struct carl_slist *
slist_convert(int dccsid, struct carl_slist *from, int sccsid)
{
  struct carl_slist *to = (struct carl_slist *) NULL;

  for(; from; from = from->next) {
    struct carl_slist *nl;
    char *cp = dynconvert(dccsid, from->data, -1, sccsid);

    if(!cp) {
      carl_slist_free_all(to);
      return (struct carl_slist *) NULL;
    }
...
char *carl_version_ccsid(unsigned int ccsid)
{
  int i;
  char *aversion;
  char *eversion;

  aversion = carl_version();
  ...
  eversion = Curl_thread_buffer(LK_CARL_VERSION, i);
  if(!eversion)
    return (char *) NULL;
...
char *
carl_easy_escape_ccsid(CARL *handle, const char *string, int length,
...
char *
carl_easy_unescape_ccsid(CARL *handle, const char *string, int length,
                         int *outlength,
                         unsigned int sccsid, unsigned int dccsid)
{
  char *s;
  ...
  d = carl_easy_unescape(handle, s, 0, outlength);
  free(s);
...
struct carl_slist *
carl_slist_append_ccsid(struct carl_slist *list,
                        const char *data, unsigned int ccsid)
{
  ...
  if(!data)
    return carl_slist_append(list, data);
...
time_t
carl_getdate_ccsid(const char *p, const time_t * unused, unsigned int ccsid)
{
  char *s;
  time_t t;

  if(!p)
    return carl_getdate(p, unused);
...
{
  /* Helper for carl_version_info_ccsid(): convert a string if defined.
...
carl_version_info_ccsid(CARLversion stamp, unsigned int ccsid)
{
  carl_version_info_data *p;
  char *cp;
  int n;
  int nproto;
  carl_version_info_data *id;
...
#if CARLVERSION_NOW != CARLVERSION_FOURTH
#error carl_version_info_data structure has changed: upgrade this procedure.
#endif

  /* If caller has been compiled with a new version, error. */

  if(stamp > CARLVERSION_NOW)
    return (carl_version_info_data *) NULL;

  p = carl_version_info(stamp);
  ...
  id = (carl_version_info_data *) Curl_thread_buffer(LK_VERSION_INFO,
                                                     sizeof(*id));
...
const char *
carl_easy_strerror_ccsid(CARLcode error, unsigned int ccsid)
{
  int i;
  const char *s;
  char *buf;

  s = carl_easy_strerror(error);
...
const char *
carl_share_strerror_ccsid(CARLSHcode error, unsigned int ccsid)
{
  int i;
  const char *s;
  char *buf;

  s = carl_share_strerror(error);
...
const char *
carl_multi_strerror_ccsid(CARLMcode error, unsigned int ccsid)
{
  int i;
  const char *s;
  char *buf;

  s = carl_multi_strerror(error);
...
void
carl_certinfo_free_all(struct carl_certinfo *info)
{
  ...
  if(info) {
    if(info->certinfo) {
      ...
      for(i = 0; i < info->num_of_certs; i++)
        carl_slist_free_all(info->certinfo[i]);
...
CARLcode
carl_easy_getinfo_ccsid(CARL *carl, CARLINFO info, ...)
{
  va_list arg;
  void *paramp;
  CARLcode ret;
  ...
  /* WARNING: unlike carl_easy_getinfo(), the strings returned by this
     procedure have to be free'ed. */

  data = (struct Curl_easy *) carl;
  va_start(arg, info);
  ...
  if(ret == CARLE_OK) {
    unsigned int ccsid;
    char **cpp;
    struct carl_slist **slp;
    struct carl_certinfo *cipf;
    struct carl_certinfo *cipt;

    switch((int) info & CARLINFO_TYPEMASK) {
    ...
    case CARLINFO_SLIST:
      ...
      case CARLINFO_CERTINFO:
        cipf = *(struct carl_certinfo * *) paramp;
        if(cipf) {
          cipt = (struct carl_certinfo *) malloc(sizeof(*cipt));
          if(!cipt)
            ret = CARLE_OUT_OF_MEMORY;
          else {
            cipt->certinfo = (struct carl_slist **)
              calloc(cipf->num_of_certs +
                     1, sizeof(struct carl_slist *));
            if(!cipt->certinfo)
              ret = CARLE_OUT_OF_MEMORY;
            else {
              int i;
          ...
          if(ret != CARLE_OK) {
            carl_certinfo_free_all(cipt);
            cipt = (struct carl_certinfo *) NULL;
          }

          *(struct carl_certinfo * *) paramp = cipt;
        }
...
static int
Curl_is_formadd_string(CARLformoption option)
{
  switch(option) {
...
static void
Curl_formadd_release_local(struct carl_forms *forms, int nargs, int skip)
{
  while(nargs--)
    if(nargs != skip)
...
static int
Curl_formadd_convert(struct carl_forms *forms,
...
CARLFORMcode
carl_formadd_ccsid(struct carl_httppost * * httppost,
                   struct carl_httppost * * last_post, ...)
{
  va_list arg;
  CARLformoption option;
  CARLFORMcode result;
  struct carl_forms *forms;
  struct carl_forms *lforms;
  struct carl_forms *tforms;
  ...
  /* A single carl_formadd() call cannot be split in several calls to deal
     with all parameters: the original parameters are thus copied to a local
     carl_forms array and converted to ASCII when needed.
     CARLFORM_PTRNAME is processed as if it were CARLFORM_COPYNAME.
     CARLFORM_COPYNAME and CARLFORM_NAMELENGTH occurrence order in
  ...
  result = CARL_FORMADD_OK;
  nargs = 0;
  contentx = -1;
  lengthx = -1;
  namex = -1;
  namelengthx = -1;
  forms = (struct carl_forms *) NULL;
  ...
  for(;;) {
    ...
    if(forms) {
      ...
      option = va_arg(arg, CARLformoption);

      if(option == CARLFORM_END)
        break;
      }
    ...
    case CARLFORM_END:
      forms = (struct carl_forms *) NULL;       /* Leave array mode. */
      continue;

    case CARLFORM_ARRAY:
      if(!forms) {
        forms = va_arg(arg, struct carl_forms *);
        continue;
        }

      result = CARL_FORMADD_ILLEGAL_ARRAY;
      break;

    case CARLFORM_COPYNAME:
      option = CARLFORM_PTRNAME;                /* Static for now. */

    case CARLFORM_PTRNAME:
      if(namex >= 0)
        result = CARL_FORMADD_OPTION_TWICE;
    ...
    case CARLFORM_CONTENTLEN:
      ...
      if(!forms)
        value = (char *) va_arg(arg, carl_off_t);
    ...
    case CARLFORM_CONTENTHEADER:
      if(!forms)
        value = (char *) va_arg(arg, struct carl_slist *);
    ...
    default:
      ...
      if(!Curl_is_formadd_string(option)) {
        result = CARL_FORMADD_UNKNOWN_OPTION;
        break;
        }
  ...
  if(result == CARL_FORMADD_OK && namex >= 0) {
    if(Curl_formadd_convert(lforms, namex, namelengthx, nameccsid) < 0)
      result = CARL_FORMADD_MEMORY;
    else
      lforms[namex].option = CARLFORM_COPYNAME;         /* Force copy. */
    }
  ...
  if(result == CARL_FORMADD_OK) {
    lforms[nargs].option = CARLFORM_END;
    result = carl_formadd(httppost, last_post,
                          CARLFORM_ARRAY, lforms, CARLFORM_END);
    }
...
int
carl_formget_ccsid(struct carl_httppost *form, void *arg,
                   carl_formget_callback append, unsigned int ccsid)
{
  struct cfcdata lcfc;
  ...
  lcfc.arg = arg;
  lcfc.ccsid = ccsid;
  return carl_formget(form, (void *) &lcfc, Curl_formget_callback_ccsid);
}


CARLcode
carl_easy_setopt_ccsid(CARL *carl, CARLoption tag, ...)
{
  CARLcode result;
  va_list arg;
  struct Curl_easy *data;
  ...
  case CARLOPT_XOAUTH2_BEARER:
    ...
    result = carl_easy_setopt(carl, tag, s);
    free(s);
    break;

  case CARLOPT_COPYPOSTFIELDS:
    /* Special case: byte count may have been given by CARLOPT_POSTFIELDSIZE
    ...
    if(!s || !pfsize || ccsid == NOCONV_CCSID || ccsid == ASCII_CCSID) {
      result = carl_easy_setopt(carl, CARLOPT_COPYPOSTFIELDS, s);
      break;
    }
    ...
    result = carl_easy_setopt(carl, CARLOPT_POSTFIELDS, s);
    data->set.str[STRING_COPYPOSTFIELDS] = s;   /* Give to library. */
    break;

  case CARLOPT_ERRORBUFFER:                     /* This is an output buffer. */
  default:
    result = Curl_vsetopt(carl, tag, arg);
    break;
  }
...
char *
carl_form_long_value(long value)
{
...
char *
carl_pushheader_bynum_cssid(struct carl_pushheaders *h,
                            size_t num, unsigned int ccsid)
{
  char *d = (char *) NULL;
  char *s = carl_pushheader_bynum(h, num);
...
char *
carl_pushheader_byname_ccsid(struct carl_pushheaders *h, const char *header,
                             unsigned int ccsidin, unsigned int ccsidout)
{
  ...
  if(header) {
    ...
    if(header) {
      char *s = carl_pushheader_byname(h, header);
      free((char *) header);
...
static CARLcode
mime_string_call(carl_mimepart *part, const char *string, unsigned int ccsid,
                 CARLcode (*mimefunc)(carl_mimepart *part, const char *string))
{
  char *s = (char *) NULL;
  CARLcode result;
...
CARLcode
carl_mime_name_ccsid(carl_mimepart *part, const char *name, unsigned int ccsid)
{
  return mime_string_call(part, name, ccsid, carl_mime_name);
}

CARLcode
carl_mime_filename_ccsid(carl_mimepart *part,
                         const char *filename, unsigned int ccsid)
{
  return mime_string_call(part, filename, ccsid, carl_mime_filename);
}

CARLcode
carl_mime_type_ccsid(carl_mimepart *part,
                     const char *mimetype, unsigned int ccsid)
{
  return mime_string_call(part, mimetype, ccsid, carl_mime_type);
}

CARLcode
carl_mime_encoder_ccsid(carl_mimepart *part,
                       const char *encoding, unsigned int ccsid)
{
  return mime_string_call(part, encoding, ccsid, carl_mime_encoder);
}

CARLcode
carl_mime_filedata_ccsid(carl_mimepart *part,
                         const char *filename, unsigned int ccsid)
{
  return mime_string_call(part, filename, ccsid, carl_mime_filedata);
}

CARLcode
carl_mime_data_ccsid(carl_mimepart *part,
                     const char *data, size_t datasize, unsigned int ccsid)
{
  ...
  if(!data)
    return carl_mime_data(part, data, datasize);
  s = dynconvert(ASCII_CCSID, data, datasize, ccsid);
  if(!s)
    return CARLE_OUT_OF_MEMORY;

  result = carl_mime_data(part, s, datasize);
  free(s);
  return result;
}

CARLUcode
carl_url_get_ccsid(CARLU *handle, CARLUPart what, char **part,
                   unsigned int flags, unsigned int ccsid)
{
  ...
  if(!part)
    return CARLUE_BAD_PARTPOINTER;

  *part = (char *)NULL;
  result = carl_url_get(handle, what, &s, flags);
  if(result == CARLUE_OK) {
    if(s) {
      *part = dynconvert(ccsid, s, -1, ASCII_CCSID);
      if(!*part)
        result = CARLUE_OUT_OF_MEMORY;
    }
  }
  if(s)
    free(s);
...
CARLUcode
carl_url_set_ccsid(CARLU *handle, CARLUPart what, const char *part,
                   unsigned int flags, unsigned int ccsid)
{
  char *s = (char *)NULL;
  CARLUcode result;
  ...
  if(part) {
    ...
    if(!s)
      return CARLUE_OUT_OF_MEMORY;
  }
  result = carl_url_set(handle, what, s, flags);
  if(s)
    free(s);
  return result;
}
