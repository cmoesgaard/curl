...
dnl CARL_CHECK_OPENSSL_API_HEADERS
dnl -------------------------------------------------
...
AC_DEFUN([CARL_CHECK_OPENSSL_API_HEADERS], [
  #
  tst_api="unknown"
  #
  AC_MSG_CHECKING([for OpenSSL headers version])
  CARL_CHECK_DEF([OPENSSL_VERSION_NUMBER], [
#   ifdef USE_OPENSSL
...
#   endif
  ...
  if test "$carl_cv_have_def_OPENSSL_VERSION_NUMBER" = "yes"; then
    tst_verlen=`expr "$carl_cv_def_OPENSSL_VERSION_NUMBER" : '.*'`
    case "x$tst_verlen" in
      x6)
        tst_vermaj=`echo $carl_cv_def_OPENSSL_VERSION_NUMBER | cut -c 3`
        tst_vermin=`echo $carl_cv_def_OPENSSL_VERSION_NUMBER | cut -c 4`
        tst_verfix=`echo $carl_cv_def_OPENSSL_VERSION_NUMBER | cut -c 5`
        tst_api=0x$tst_vermaj$tst_vermin$tst_verfix
        ;;
      x11|x10)
        tst_vermaj=`echo $carl_cv_def_OPENSSL_VERSION_NUMBER | cut -c 3`
        tst_vermin=`echo $carl_cv_def_OPENSSL_VERSION_NUMBER | cut -c 5`
        tst_verfix=`echo $carl_cv_def_OPENSSL_VERSION_NUMBER | cut -c 7`
        tst_api=0x$tst_vermaj$tst_vermin$tst_verfix
        ;;
      *)
        tst_api="unknown"
    ...
    tst_show="$tst_show - $carl_cv_def_OPENSSL_VERSION_NUMBER"
  else
    tst_show="unknown"
  fi
...
dnl fi
  carl_openssl_api_headers=$tst_api
])


dnl CARL_CHECK_OPENSSL_API_LIBRARY
dnl -------------------------------------------------
...
AC_DEFUN([CARL_CHECK_OPENSSL_API_LIBRARY], [
  #
  tst_api="unknown"
  #
...
dnl    No matter what, do not ever define this manually or by any other means.])
dnl fi
  carl_openssl_api_library=$tst_api
])


dnl CARL_CHECK_OPENSSL_API
dnl -------------------------------------------------

AC_DEFUN([CARL_CHECK_OPENSSL_API], [
  #
  CARL_CHECK_OPENSSL_API_HEADERS
  CARL_CHECK_OPENSSL_API_LIBRARY
  #
  tst_match="yes"
  #
  AC_MSG_CHECKING([for OpenSSL headers and library versions matching])
  if test "$carl_openssl_api_headers" = "unknown" ||
    test "$carl_openssl_api_library" = "unknown"; then
    tst_match="fail"
    tst_warns="Can not compare OpenSSL headers and library versions."
  elif test "$carl_openssl_api_headers" != "$carl_openssl_api_library"; then
    tst_match="no"
