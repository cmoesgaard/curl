...
dnl CARL_CHECK_OPTION_ARES
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-ares or --disable-ares, and
dnl set shell variable want_ares as appropriate.

AC_DEFUN([CARL_CHECK_OPTION_ARES], [
dnl   AC_BEFORE([$0],[CARL_CHECK_OPTION_THREADS])dnl
  AC_BEFORE([$0],[CARL_CHECK_LIB_ARES])dnl
...
dnl CARL_CHECK_OPTION_CARLDEBUG
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-carldebug or --disable-carldebug, and set
dnl shell variable want_carldebug value as appropriate.

AC_DEFUN([CARL_CHECK_OPTION_CARLDEBUG], [
  AC_BEFORE([$0],[CARL_CHECK_CARLDEBUG])dnl
  AC_MSG_CHECKING([whether to enable carl debug memory tracking])
  OPT_CARLDEBUG_BUILD="default"
  AC_ARG_ENABLE(carldebug,
AC_HELP_STRING([--enable-carldebug],[Enable carl debug memory tracking])
AC_HELP_STRING([--disable-carldebug],[Disable carl debug memory tracking]),
  OPT_CARLDEBUG_BUILD=$enableval)
  case "$OPT_CARLDEBUG_BUILD" in
    no)
      dnl --disable-carldebug option used
      want_carldebug="no"
      AC_MSG_RESULT([no])
      ;;
    default)
      dnl configure's carldebug option not specified. Initially we will
      dnl handle this as a request to use the same setting as option
      dnl --enable-debug. IOW, initially, for debug-enabled builds
      dnl this will be handled as a request to enable carldebug if
      dnl possible, and for debug-disabled builds this will be handled
      dnl as a request to disable carldebug.
      if test "$want_debug" = "yes"; then
        AC_MSG_RESULT([(assumed) yes])
        AC_DEFINE(CARLDEBUG, 1, [to enable carl debug memory tracking])
      else
        AC_MSG_RESULT([no])
      fi
      want_carldebug_assumed="yes"
      want_carldebug="$want_debug"
      ;;
    *)
      dnl --enable-carldebug option used.
      dnl The use of this option value is a request to enable carl's
      dnl debug memory tracking for the libcarl library. This can only
      ...
      dnl --disable-carldebug had been given setting shell variable
      dnl want_carldebug to 'no'.
      want_carldebug="yes"
      AC_DEFINE(CARLDEBUG, 1, [to enable carl debug memory tracking])
      AC_MSG_RESULT([yes])
      ;;
...
dnl CARL_CHECK_OPTION_DEBUG
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-debug or --disable-debug, and set shell
dnl variable want_debug value as appropriate.

AC_DEFUN([CARL_CHECK_OPTION_DEBUG], [
  AC_BEFORE([$0],[CARL_CHECK_OPTION_WARNINGS])dnl
  AC_BEFORE([$0],[CARL_CHECK_OPTION_CARLDEBUG])dnl
...
dnl CARL_CHECK_OPTION_SYMBOL_HIDING
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-symbol-hiding or --disable-symbol-hiding,
dnl setting shell variable want_symbol_hiding value.

AC_DEFUN([CARL_CHECK_OPTION_SYMBOL_HIDING], [
  AC_BEFORE([$0],[CARL_CHECK_COMPILER_SYMBOL_HIDING])dnl
...
dnl CARL_CHECK_OPTION_THREADS
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-threads or --disable-threads, and
dnl set shell variable want_threads as appropriate.

dnl AC_DEFUN([CARL_CHECK_OPTION_THREADS], [
dnl   AC_BEFORE([$0],[CARL_CHECK_LIB_THREADS])dnl
...
dnl CARL_CHECK_OPTION_RT
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --disable-rt and set shell variable dontwant_rt
dnl as appropriate.

AC_DEFUN([CARL_CHECK_OPTION_RT], [
  AC_BEFORE([$0], [CARL_CHECK_LIB_THREADS])dnl
...
dnl CARL_CHECK_OPTION_WARNINGS
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-warnings or --disable-warnings, and set
dnl shell variable want_warnings as appropriate.

AC_DEFUN([CARL_CHECK_OPTION_WARNINGS], [
  AC_REQUIRE([CARL_CHECK_OPTION_DEBUG])dnl
  AC_BEFORE([$0],[CARL_CHECK_OPTION_WERROR])dnl
  AC_BEFORE([$0],[XC_CHECK_PROG_CC])dnl
...
dnl CARL_CHECK_OPTION_WERROR
dnl -------------------------------------------------
dnl Verify if configure has been invoked with option
dnl --enable-werror or --disable-werror, and set
dnl shell variable want_werror as appropriate.

AC_DEFUN([CARL_CHECK_OPTION_WERROR], [
  AC_BEFORE([$0],[CARL_CHECK_COMPILER])dnl
...
dnl CARL_CHECK_NONBLOCKING_SOCKET
dnl -------------------------------------------------
dnl Check for how to set a socket into non-blocking state.

AC_DEFUN([CARL_CHECK_NONBLOCKING_SOCKET], [
  AC_REQUIRE([CARL_CHECK_FUNC_FCNTL])dnl
  AC_REQUIRE([CARL_CHECK_FUNC_IOCTL])dnl
  AC_REQUIRE([CARL_CHECK_FUNC_IOCTLSOCKET])dnl
  AC_REQUIRE([CARL_CHECK_FUNC_IOCTLSOCKET_CAMEL])dnl
  AC_REQUIRE([CARL_CHECK_FUNC_SETSOCKOPT])dnl
  #
  tst_method="unknown"

  AC_MSG_CHECKING([how to set a socket into non-blocking mode])
  if test "x$carl_cv_func_fcntl_o_nonblock" = "xyes"; then
    tst_method="fcntl O_NONBLOCK"
  elif test "x$carl_cv_func_ioctl_fionbio" = "xyes"; then
    tst_method="ioctl FIONBIO"
  elif test "x$carl_cv_func_ioctlsocket_fionbio" = "xyes"; then
    tst_method="ioctlsocket FIONBIO"
  elif test "x$carl_cv_func_ioctlsocket_camel_fionbio" = "xyes"; then
    tst_method="IoctlSocket FIONBIO"
  elif test "x$carl_cv_func_setsockopt_so_nonblock" = "xyes"; then
...
dnl CARL_CONFIGURE_SYMBOL_HIDING
dnl -------------------------------------------------
...
AC_DEFUN([CARL_CONFIGURE_SYMBOL_HIDING], [
  AC_MSG_CHECKING([whether hiding of library internal symbols will actually happen])
  CFLAG_CARL_SYMBOL_HIDING=""
  doing_symbol_hiding="no"
  if test x"$carl_cv_native_windows" != "xyes" &&
    test "$want_symbol_hiding" = "yes" &&
    test "$supports_symbol_hiding" = "yes"; then
    doing_symbol_hiding="yes"
    CFLAG_CARL_SYMBOL_HIDING="$symbol_hiding_CFLAGS"
    AC_DEFINE_UNQUOTED(CARL_EXTERN_SYMBOL, $symbol_hiding_EXTERN,
      [Definition to make a library symbol externally visible.])
    AC_MSG_RESULT([yes])
  else
    AC_MSG_RESULT([no])
  fi
  AM_CONDITIONAL(DOING_CARL_SYMBOL_HIDING, test x$doing_symbol_hiding = xyes)
  AC_SUBST(CFLAG_CARL_SYMBOL_HIDING)
])


dnl CARL_CHECK_LIB_ARES
dnl -------------------------------------------------
...
AC_DEFUN([CARL_CHECK_LIB_ARES], [
  ...
  if test "$want_ares" = "yes"; then
    ...
    if test -n "$want_ares_path"; then
      ...
      CARL_CHECK_PKGCONFIG(libcares, [$ARES_PCDIR])
      if test "$PKGCONFIG" != "no" ; then
        ares_LIBS=`CARL_EXPORT_PCDIR([$ARES_PCDIR])
          $PKGCONFIG --libs-only-l libcares`
        ares_LDFLAGS=`CARL_EXPORT_PCDIR([$ARES_PCDIR])
          $PKGCONFIG --libs-only-L libcares`
        ares_CPPFLAGS=`CARL_EXPORT_PCDIR([$ARES_PCDIR])
      ...
      if test -d "$srcdir/ares"; then
        dnl c-ares sources embedded in carl tree
        embedded_ares="yes"
...
AC_DEFUN([CARL_CHECK_OPTION_NTLM_WB], [
  AC_BEFORE([$0],[CARL_CHECK_NTLM_WB])dnl
  OPT_NTLM_WB="default"
...
AC_DEFUN([CARL_CHECK_OPTION_ECH], [
...
AC_HELP_STRING([--disable-ech],[Disable ECH support]),
  ...
  case "$OPT_ECH" in
    ...
    *)
      ...
      want_ech="yes"
      carl_ech_msg="enabled (--disable-ech)"
      experimental="ech"
      AC_MSG_RESULT([yes])
      ;;
  esac
